...
let recvtimeout (ntwrk:Unix.file_descr) recv_packet num1 num2 =
  ...
  in
  if List.mem ntwrk r1 
  then
    let (sz, oth) = (* Back to UDP communication
             read ntwrk recv_packet num1 num2 *)
            recvfrom ntwrk recv_packet num1 num2 []
    in
    (sz, oth, RCVOK)
  else
...
let rec do_client_step = fun
  ...
  let ressent = (* we send out packet *)
    (match pci with
     | IPacket (pld, plen) ->
        print_string "Sending out packet:\n" ; 
        (* back to UDP communication
        write conn (Bytes.of_string pld) 0 (hash plen)
         *)
        print_string pld ;
        print_newline ();
        sendto conn (Bytes.of_string pld) 0 (hash plen) [] saddr
     | Init (_, _, _, _, _, _) -> -1
     | PIError _ -> -1)
  in
  if ressent < 0
  then (* after unsuccessful packet sending next protocol step gets error *)
    do_client_step next_step_op recv_packet
      (SError "Error in sending of packet to remote server",
       PIError "Error in sending of packet to remote server") saddr conn
  else (* after successful packet sending do next step *)
    let (sz, otherside, status) = recvtimeout conn recv_packet 0 516 in
    let (tm, plen) = (match status with
                      | RCVOK -> (false, of_int sz)
                      | RCVTIMEOUT -> (true, of_int 0)) in
    let rcvd = my_string_of_bytes recv_packet in
    let (nst, pout) =
      print_string "Received data: ";
      print_string rcvd;
      print_newline ();
      next_step_op st (IPacket (rcvd, plen)) in 
    match pout with
    | Nothing -> OK
    | OPacket content ->
       do_client_step next_step_op recv_packet
...
let client_loop
      ...
      (port:int) =
  print_string "Entered protocol loop\n";
...
let recv_srvr (ntwrk:file_descr list) recv_packet num1 num2 =
  ...
  | hd :: tl ->
     if List.mem hd r1 
     then
       let (sz, oth) = recvfrom hd recv_packet num1 num2 []
       in
       (sz, oth)
     else
       (0, nulladdr)
...
let print_addr addr =
  (match addr with
   ...
   | ADDR_INET (addr, n) ->
      print_string ("INET addr: " ^
                      (string_of_inet_addr addr) ^ ":" ^ (string_of_int n)));
;;
...
let get_conns_from_st st =
  ...
  | SNoOp (conn, addr) ->
     let _ =
       ...
       print_addr addr;
       print_newline ();
       bind conn addr in [conn]
  | SFComm ((((_, fds), _), c), _) -> c :: fds
  | _ -> []
;;

(** The function that handles the main protocol loop on the server side.
    Note that we do not need st_pc pair present in the client side case.
    This piece of data is triggered by the input from the network.
    + next_step_op - the next step operation according to the protocol
    + packet_buffer - buffer for incoming packets
    + st_pc - pair with the internal state of the server and input data
              to the server
 *)
let rec do_server_step = fun
  ...
  print_newline ();
  if size < 0
  then (* after unsuccessful packet receive, we just repeat *)
    (print_string "Error in packet receive\n";
     do_server_step next_step_op packet_buffer st_spi)
  else (* after successful packet receive we do next step *)
    ( let strfbuf = Bytes.to_string packet_buffer in
      print_string "Received packet: " ;
      print_string strfbuf;
      print_newline ();
     let (nst, pout) =
       next_step_op st (SIPacket (addr, (Bytes.to_string packet_buffer))) in 
     match nst with
     | SNoOp _ -> (* Initial state is always outside the loop so
                      this is error. *)
        RunError "Unexpected initial state in the middle of protocol"
     | SFinal -> (* We successfully entered the final state so the final
                    result is OK. This should not happen. *)
        OK
     | SSError reason -> (* Error means the final result is error. *)
        RunError reason
     | SFComm (pdata, _) -> (* Further communication should take place. *)
        (* We try to send out data back to client *)
        match conns with
        ...
        | hd :: tl ->
           ...
           | OPacket content ->
              let _ =
                blit_string content 0
                  packet_buffer 0 (String.length content) in
              let (* We send data  it out here. *)
                status =
                print_string "Sending out: " ;
                print_string content ;
                print_newline () ;
                sendto hd packet_buffer 0 (String.length content) [] addr in
              if status > 0
              then
                let st_pi = (nst, SNothing) in
                do_server_step next_step_op packet_buffer st_pi
...
let srvr_loop
      ...
      (server_addr:inet_addr)
      (port:int) =
  print_string "Entered server protocol loop\n";
  let portaddr = ADDR_INET(server_addr, port) in
  let buffer_size = 1024 in
  let recv_packet = Bytes.make buffer_size '\000' in
