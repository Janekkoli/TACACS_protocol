...
(* string to nat functions from stack https://stackoverflow.com/questions/54752598/transfering-proof-from-z-to-n-in-coq *)



Definition parse_request (s : string) : option request :=
  let splitted_s := splitonCRLFandSpaces s in 
  match splitted_s with
  | [["1";"AUTH";params];[user];[pass];[stringLine]] => match num_prefix_and_length stringLine with
                                        | Some (line, _) =>  Some  (Auth {|
                                          Auth.username := user;
                                          Auth.password := pass;
                                          Auth.line := Z.to_nat line;
                                          Auth.style := params
                                        |})
                                        | _ => None
                                        end
